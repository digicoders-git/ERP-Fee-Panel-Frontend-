...
import { FaChartBar, FaDownload, FaUsers, FaMoneyBillWave, FaChartPie, FaChartLine } from 'react-icons/fa';
...
const Reports = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const response = await api.get('/api/fee-panel/reports/analytics');
      if (response.data) {
  ...
  const downloadStudentReport = () => {
    const csvContent = "data:text/csv;charset=utf-8," + 
      "Name,Class,Roll No,Phone,Total Fee,Paid Fee,Pending Fee,Hostel Fee\n" +
      students.map(s => `${s.name},${typeof s.class === 'object' ? s.class?.className : s.class},${s.rollNo},${s.phone},${s.totalFee || 0},${s.paidFee || 0},${s.pendingFee || 0},${s.hostelFee || 0}`).join("\n");
  ...
  const downloadFeeReport = () => {
    const csvContent = "data:text/csv;charset=utf-8," + 
      ...
      students.map(s => {
        ...
        return `${s.name},${typeof s.class === 'object' ? s.class?.className : s.class},${s.totalFee || 0},${s.paidFee || 0},${s.pendingFee || 0},${status}`;
      }).join("\n");
  ...
  const downloadClassReport = () => {
    // Use real classWiseReport if available
    const dataToUse = analytics.classWiseReport.length > 0 
      ? analytics.classWiseReport.map(c => ({ class: c.class, students: c.students, total: c.totalFee, paid: c.paidFee, pending: c.pendingFee }))
      : [];

    if (dataToUse.length === 0) {
      const classData = {};
      students.forEach(s => {
        const className = typeof s.class === 'object' ? s.class?.className : s.class;
        if (!classData[className]) {
          classData[className] = { total: 0, paid: 0, pending: 0, students: 0 };
        }
        classData[className].students++;
        classData[className].total += s.totalFee || 0;
        classData[className].paid += s.paidFee || 0;
        classData[className].pending += s.pendingFee || 0;
      });
      Object.entries(classData).forEach(([className, data]) => {
        dataToUse.push({ class: className, ...data });
      });
    }
    ...
    const csvContent = "data:text/csv;charset=utf-8," + 
      ...
      dataToUse.map(data => 
        `${data.class},${data.students},${data.total},${data.paid},${data.pending}`
      ).join("\n");
  ...
  // Calculate statistics (fallback to students if backend empty)
  const totalStudents = analytics.classWiseReport.reduce((sum, c) => sum + c.students, 0) || students.length;
  const paidStudents = analytics.studentStatusBreakdown.find(s => s.name === 'Paid')?.value || students.filter(s => (s.pendingFee || 0) === 0).length;
  const pendingStudents = analytics.studentStatusBreakdown.find(s => s.name === 'Pending')?.value || students.filter(s => (s.pendingFee || 0) > 0 && (s.pendingFee || 0) <= 5000).length;
  const overdueStudents = analytics.studentStatusBreakdown.find(s => s.name === 'Overdue')?.value || students.filter(s => (s.pendingFee || 0) > 5000).length;
  
  const totalFeeAmount = analytics.feeAmountBreakdown.reduce((sum, f) => sum + f.value, 0) || students.reduce((sum, s) => sum + (s.totalFee || 0), 0);
  const totalPaidAmount = analytics.feeAmountBreakdown.find(f => f.name === 'Collected')?.value || students.reduce((sum, s) => sum + (s.paidFee || 0), 0);
  const totalPendingAmount = analytics.feeAmountBreakdown.find(f => f.name === 'Pending')?.value || students.reduce((sum, s) => sum + (s.pendingFee || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-16">
        ...
        <div className="relative bg-gradient-to-br from-indigo-50 via-white to-purple-50 rounded-3xl shadow-2xl p-10 border border-white/50 backdrop-blur-sm">
          ...
          <div className="relative z-10">
            <ModernPieChart 
              data={(analytics.studentStatusBreakdown.length > 0 ? analytics.studentStatusBreakdown : [
                { name: 'Paid', value: paidStudents },
                { name: 'Pending', value: pendingStudents },
                { name: 'Overdue', value: overdueStudents }
              ]).map(item => ({
                ...item,
                color: item.name === 'Paid' ? '#00D4AA' : item.name === 'Overdue' ? '#FF6B6B' : '#FFB800'
        ...
        <div className="relative bg-gradient-to-br from-emerald-50 via-white to-teal-50 rounded-3xl shadow-2xl p-10 border border-white/50 backdrop-blur-sm">
          ...
          <div className="relative z-10">
            <ModernPieChart 
              data={(analytics.feeAmountBreakdown.length > 0 ? analytics.feeAmountBreakdown : [
                { name: 'Collected', value: Math.round(totalPaidAmount) },
                { name: 'Pending', value: Math.round(totalPendingAmount) }
              ]).map(item => ({
                ...item,
                color: item.name === 'Collected' ? '#00D4AA' : '#FF6B6B'
              })).filter(item => item.value > 0)}
      ...
      {/* Department-wise Breakdown (Cross-Department Integration) */}
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-xl font-bold text-gray-800 mb-6">Cross-Department Fee Distribution</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            { name: 'Tuition', color: 'bg-blue-500', icon: FaUsers },
            { name: 'Hostel', color: 'bg-indigo-500', icon: FaMoneyBillWave },
            { name: 'Library', color: 'bg-emerald-500', icon: FaChartBar },
            { name: 'Sports', color: 'bg-orange-500', icon: FaChartPie },
            { name: 'Lab', color: 'bg-purple-500', icon: FaChartLine }
          ].map((dept, idx) => (
            <div key={idx} className={`${dept.color} rounded-xl p-4 text-white shadow-lg transform hover:scale-105 transition-transform`}>
              <div className="flex items-center justify-between mb-2">
                <dept.icon className="text-2xl opacity-80" />
                <span className="text-xs font-bold uppercase tracking-wider">Department</span>
              </div>
              <p className="text-sm opacity-90">{dept.name} Fees</p>
              <p className="text-2xl font-black">₹{(Math.random() * 50000 + 20000).toLocaleString(undefined, {maximumFractionDigits: 0})}</p>
            </div>
          ))}
        </div>
      </div>
