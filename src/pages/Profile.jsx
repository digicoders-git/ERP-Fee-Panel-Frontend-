...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await api.get('/api/fee-panel/fee-admin/profile');
        if (res.data && res.data.admin) {
          const admin = res.data.admin;
          const mapped = {
            name: admin.name || 'Fee Administrator',
            email: admin.email,
            phone: admin.phone || '',
            designation: admin.designation || 'Accountant',
            department: admin.department || 'Finance',
            joinDate: admin.joinDate ? admin.joinDate.split('T')[0] : new Date().toISOString().split('T')[0],
            address: admin.address || 'School Campus',
  ...
  const handleSave = async () => {
    try {
      // The backend accepts 'email' currently. 
      await api.put('/api/fee-panel/fee-admin/profile', { email: tempData.email });
