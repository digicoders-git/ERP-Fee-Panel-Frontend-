...
const AddStudentModal = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onAdd(formData);
    setFormData({ name: '', class: '', rollNo: '', phone: '', email: '', address: '' });
    onClose();
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex space-x-3 pt-4">
            <button
              type="submit"
              className="flex-1 cursor-pointer bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600 transition-colors"
            >
              Add Student
            </button>
            <button
